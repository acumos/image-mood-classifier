...
/**
 image-mood-classes.js - send frames to an image classification service
...
function genClassTable (data, div) {
	...
	var scoreBoost = 0.45; // give classes some visual oomph
	$(div).empty();
    ...
    if ('results' in data) {
        $.each(data.results.tags, function(k, v) {
            if (count < limit && v.score >= minScore) {
                var fade = (v.score > 1.0) ? 1 : v.score + scoreBoost;	// fade out low confidence classes
